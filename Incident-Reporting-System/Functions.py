...
def update_incident_status(conn):
    """
    Uppdaterar incidentstatus via stored procedure:
    CALL update_incident_status(incident_id, new_status_id, guard_id, note)
    Triggern loggar automatiskt till incident_status_history.
    """
    cursor = conn.cursor()
    try:
        ...
        cursor.callproc("update_incident_status", (incident_id, new_status_id, guard_id, note))
        conn.commit()
